...
import { doc, setDoc } from "firebase/firestore"; // <-- Firestore back in
...
import AppBlurHeader2 from "../../../(auth)/Pages/AppBlurHeader2";
...
const HeightComponent = memo(function HeightComponent() {
  const {
  ...
  return (
    <View style={{ width: "100%", backgroundColor: "#fff" }}>
      <Text style={{ fontSize: 28, marginTop: height(5), marginLeft: width(1), fontWeight: "700" }}>
        Height
      </Text>
      <Text style={{ fontSize: size(14), marginTop: height(1), marginLeft: width(1), fontWeight: "700", color: "#BCC1CA" }}>
        This will be used to calibrate your custom plan.
      </Text>

      {/* Unit switch */}
      <View style={{ width: "100%", alignItems: "center", marginTop: height(3) }}>
      ...
      {/* Wheels */}
      {isMetric ? (
        <View style={{ width: "100%", alignSelf: "center", top: height(2) }}>
...
export default function HealthSetupScreen({ goalId, goalIds }) {
  const {
    ...
    selectedConditions,
    goal,
    // height state for saving
    ft, inch, cm, kg, lb,
    // assuming these are provided by your context

    onSaved, // if your context passes a callback
  } = useOnboarding();
  ...
  // SAVE ONLY WHEN BUTTON PRESSED (Firestore restored)
  const onPressSave = async () => {
    if (saving) return;
   
      setSaving(true);

      // 2) update current height/weight on user root doc
      await setDoc(
  ...
  const multiple = [showDiabetes, showKidney, showHeart, showCoffee, showSmoking].filter(Boolean).length > 1;
  ...
  return (
    <>
      ...
      >
        ...
        <View style={{ width: "90%", alignSelf: "center", marginTop: height(2) }}>
          {/* Diabetes */}
          {showDiabetes && (
            <>
      ...
      >
        ...
        <Check color={"#fff"} size={18} style={{ position: 'absolute', right: width(5) }} />
      </TouchableOpacity>
    </>
  );
}
