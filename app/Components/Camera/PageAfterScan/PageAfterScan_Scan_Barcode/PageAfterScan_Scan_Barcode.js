...
export default function PageAfterScan_Scan_Barcode() {
...
const [ready, setReady] = useState(false);
  ...
  // ✅ Prefer ingredientsBreakdown (if present), otherwise fall back to list
  ...
  }, [ingredientsBreakdown, list]);

  const hasAnything = useMemo(() => {
    const items = list ?? [];
    const hasIB = Array.isArray(ingredientsBreakdown) && ingredientsBreakdown.length > 0;
    const hasPartial = (partial?.scan_summary?.ingredients?.length ?? 0) > 0;
    return items.length > 0 || hasIB || hasPartial;
  }, [list, partial, ingredientsBreakdown]);
...
useEffect(() => {
  ...
  return (
    <>
     {showLoading ? (
  <LoadingPage
    isDone={ready}                 // ✅ flip true when scan results loaded
    onDone={() => setShowLoading(false)}  // hide only after 100% animation finishes
  />
      ) : (
